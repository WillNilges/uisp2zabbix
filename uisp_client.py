...
class UISPClient():
    ...
    def get_devices(self):
        response = requests.get(f"{self.endpoint}/devices", headers=self.headers, verify=False)
    ...
    def get_data_links(self):
        response = requests.get(f"{self.endpoint}/data-links", headers=self.headers, verify=False)
